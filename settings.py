...
def validate_settings(settings):
    ...
    settings["subreddits"] = validate_subreddits(
        settings["subreddits"], settings["reddit"]
    )
...
def validate_subreddits(subreddits, reddit_credentials):
    """Validate that each subreddit exists on Reddit."""
    ...
    for subreddit in subreddits:
        if subreddit:  # Skip empty subreddits
            try:
                reddit.subreddit(subreddit).id  # Attempt to get subreddit info
                valid_subreddits.append(subreddit)
                print(f"Subreddit '{subreddit}' is valid.")
            except Exception as e:
                print(f"Warning: Subreddit '{subreddit}' is invalid. Error: {e}")

    if not valid_subreddits:
        print("No valid subreddits found. Please re-enter valid subreddits.")
